...
List<Hotel> alexHotels = [
  Hotel(
      ...
      description: "Hotels offer a temporary haven, providing comfort, convenience, and often a touch of luxury for travelers seeking respite from their journeys.",
      price: 0,url:  "https://www.hilton.com/en/hotels/cairhtw-ramses-hilton/",isFavorite: false),
  Hotel(
      ...
      description:  "From opulent suites to cozy rooms, hotels offer a spectrum of accommodations to suit every taste and budget.",
      price:  0,url:  "https://www.hilton.com/en/hotels/cairhtw-ramses-hilton/",isFavorite:  false),
  Hotel(
      ...
      description:  "Hotels welcome weary travelers with open arms, offering a sanctuary where comfort meets convenience.",
      price:  0,url:  "https://www.hilton.com/en/hotels/cairhtw-ramses-hilton/",isFavorite:  false),
  Hotel(
      ...
      description: "Indulge in a world of comfort and luxury at our exquisite hotel, where every moment is crafted to exceed your expectations and leave you with cherished memories to last a lifetime",
     price: 0,url: "https://www.hilton.com/en/hotels/cairhtw-ramses-hilton/",isFavorite:false),
  Hotel(
      ...
      description:"Experience the epitome of luxury and sophistication at Semirames Hotel, where timeless elegance meets modern comfort, creating an unforgettable oasis for discerning travelers.",
    price:0,url: "https://www.hilton.com/en/hotels/cairhtw-ramses-hilton/",isFavorite:false),
...
class _ScreenOneState extends State<ScreenOne> {
  ...
  try {
    DocumentSnapshot docSnapshot = await FirebaseFirestore.instance.collection('Usere').doc(uid).get();
    if (docSnapshot.exists) {
      setState(() {
        _firestoreUserId = docSnapshot.get('userId') as String?; // Safe cast to String?
      });
    } else {
  ...
  void toggleFavoriteStatus(int index) async {
    if (_firestoreUserId == null) return; // Cannot proceed without a Firestore user ID

    final hotel = alexHotels[index]; // Use alexHotels instead of cairoHotels
    final hotelRef = FirebaseFirestore.instance.collection('Hotels').doc(hotel.name);
    final userFavoritesRef = FirebaseFirestore.instance.collection('Usere').doc(_firestoreUserId!).collection('favorites');
    ...
    setState(() {
      alexHotels[index].isFavorite = !alexHotels[index].isFavorite; // Use alexHotels instead of cairoHotels
    });
  }
